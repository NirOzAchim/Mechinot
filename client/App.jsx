...
/* ============================================================
   קבוצות הניווט
   ------------------------------------------------------------
   ⚠⚠⚠ **קבוצה שהפריט היחיד בה נושא את שמה אינה מקבלת
     כותרת.** «לוח מודעות» מעל «לוח מודעות», ו«בקשות
     יציאה» מעל «בקשות יציאה» — הכותרת אינה מוסיפה
     מידע, והיא **מכפילה את אורך התפריט ומוסיפה רעש**.
     במסך מלא זה היה שמונה מקרים מתוך עשר.

     ⚠ ההשוואה היא על **התוויות הסופיות** ולא על מפתחות,
       כי המכינה ערכה אותן: מכינה שתקרא למודול «מודעות»
       ולמסך «לוח מודעות» כן רואה שתי שורות, וזה נכון.
   ============================================================ */
const NavGroups = ({ groups, screen, go }) => (
  <>
    {groups.map((g) => {
      const GIcon = moduleIcon(g.module);
      /* ⚠ **כל קבוצה שאחד מפריטיה נושא את שמה**, ולא רק
         קבוצה של פריט אחד. «תקלות ובעיות» מעל «תקלות
         ובעיות» + «טיפול בתקלות» הוא אותה כפילות בדיוק,
         והרווח בין הקבוצות מסמן את הגבול גם בלי כותרת. */
      const bare = g.items.some((i) => i.title === g.title);
      return (
        <div className="navgroup" key={g.module}>
          {!bare && <h4><GIcon size={12} />{g.title}</h4>}
          {g.items.map((i) => {
            const Icon = screenIcon(i.key);
            return (
              <button key={i.key} className={"navlink " + (screen === i.key ? "on" : "")}
                onClick={() => go(i.key)}>
                {/* ⚠ האייקון באריח משלו ולא ערום — זה מה
                    שנותן לעין על מה לנוח ברשימה של עשרים
                    שורות, וזה הפרט שדף הנחיתה משתמש בו בכל
                    כרטיס. */}
                <span className="nl-i"><Icon size={16} /></span>
